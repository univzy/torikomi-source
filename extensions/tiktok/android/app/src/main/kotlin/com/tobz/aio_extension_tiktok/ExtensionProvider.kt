...
class ExtensionProvider : ContentProvider() {
    ...
    companion object {
        private const val TAG = "AIO_EXT_TIKTOK"
        private const val SCRAPER_CHANNEL = "aio.extension.tiktok/scraper"
    ...
    override fun query(
        ...
        channel.invokeMethod("scrape", args, object : MethodChannel.Result {
            ...
            override fun notImplemented() {
                resultJson = """{"error":"scrape not implemented"}"""
                latch.countDown()
            }
        })
    ...
    private fun emptyCursor(): Cursor {
        val cursor = MatrixCursor(arrayOf("result"))
        cursor.addRow(arrayOf("""{"error":"context unavailable"}"""))
        return cursor
    }
